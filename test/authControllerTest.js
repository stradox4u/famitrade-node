...
describe('Auth Controller', () => {
  ...
  it('throws error on mismatch of user id on resend verification email', async () => {
    const req = {
      user: {
        id: 'loggedInUser'
      },
    ...
    const result = await authController.resendVerificationMail(req, {}, () => { })
    expect(result).to.be.an('error')
    expect(result).to.have.property('statusCode', 403)
    expect(result).to.have.property('message', 'Forbidden!')
  })
  ...
  it('calls sendVerificationEmail on matching ids', async () => {
    const spy = sinon.spy(sendEmails, 'sendVerificationEmail')
    const req = {
      user: {
        id: 'loggedInUser'
      },
